...
describe("TrendChart", () => {
	it("renders one bar per day of data", () => {
		const { container } = render(<TrendChart data={days([0, 1, 2, 3])} selectedWindowDays={2} />);
	...
	it("renders an accessible aria-label describing the chart", () => {
		const { getByRole } = render(<TrendChart data={days([0, 1])} selectedWindowDays={1} />);
		expect(getByRole("img", { name: /incidents per day, last 2 days/i })).toBeTruthy();
	});

	it("handles an all-zero dataset without throwing", () => {
		expect(() => render(<TrendChart data={days([0, 0, 0])} selectedWindowDays={1} />)).not.toThrow();
	});
});
